...
/**
 * <p>
 * This class contains all the information related to a generated grammar.
 * It mainly provides methods to check the validity of the grammar tree, and
 * to query for Patterns (regular expressions) used to match lexical tokens.
 * It also provides look-up methods for types and names of Blocks.</p>
 * <p>
...
public class ParsingGrammar
{
    ...
    //Array containing the compiled patterns corresponding (by index) to the literal strings.
    ...
    //Array containing the description Block associated (by index) with this literal.
    private Block[] descriptions;
    /**
     * Creates a new instance of ParsingGrammar given the specified Block tree.
     * This constructor calls <tt>ParsingGrammar( grammar, true )</tt>.
     * This Block tree MUST have the topology dicussed before after being
     * optimized.
     * @see ParsingGrammar#ParsingGrammar( Block, boolean )
     * @param grammar The Block tree containing the grammar description
     */
    public ParsingGrammar( Block grammar ) { this( grammar, true ); }
    /**
     * Creates a new instance of ParsingGrammar given the specified Block tree
     ...
     * processing the grammar. </p>
     * <p>
     * This class assigns numerical types in the order of definition of the
     * Blocks. Thus, the first definition will be of type zero, the next of
     * type one and so on.</p>
     * @see PEGUtility
     * @param grammar The Block tree containing the grammar description.
     * @param optimize If true, the tree will be optimized prior to processing.
     */
    public ParsingGrammar( Block grammar, boolean optimize )
    {
        ...
        try {
            ...
            {
                temp = grammar.getChild(i).getChild(0);   //The identifier Block.
        ...
        {
            throw new GrammarFormatException( "The grammar tree does not have the required structure." );
        }
        sortIdentifiers();  //Sorts the indices to the identifiers to allow binary search on them.
        checkIdentifiers(); //Checks that all identifiers are described.
        getLiterals();    //Checks and compiles the patterns (reg-exp's)
        sortLiterals();   //Sorts the indices to the literals to allow binary search on them.
    }
    /**
     ...
     * @throws GrammarFormatException If an identifier is not defined but used.
     */
    private void checkIdentifiers( Block desc )
    {
        if( desc.getType() == ParsingExpressionGrammar.IDENTIFIER )
            if( getTypeByName( desc.getSubtext() ) == -1 )
                throw new GrammarFormatException( "Identifier " + desc.getSubtext() +
    ...
    private void getLiterals()
    {
        ...
        for( int i = 0; i < literals.size(); i++ )
            try {
                ...
                throw new GrammarFormatException( "The pattern '" + literals.get(i) + "' is invalid." );
            }
    }
    /**
     ...
     */
    public int getNumDescriptions() { return descriptions.length; }
    /**
     ...
     * @param type The numerical type of the description to obtain.
     */
     ...
     * @param name The name of the type to obtain the description of.
     ...
     * @param literal The literal to add to the grammar.
     */
     ...
     * white spaces).
     * @param literal The string representation of the Pattern.
    ...
    public Pattern getPattern( String literal )
    {
        ...
        while( begin <= end )
        {
            ...
            else //if( literal.compareTo( literals[middle] ) == 0 )
     ...
     * @param type The numerical type to get the name of.
     ...
     * @param name The String name of the type to get the type.
    ...
    public int getTypeByName( String name )
    {
        ...
        while( begin <= end )
        {
            ...
            else //if( name.compareTo( identifiers[middle] ) == 0 )
