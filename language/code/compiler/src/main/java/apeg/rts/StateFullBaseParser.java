 ...
 * A rule is started wiht the startRule() command an can end one of the two ways: 1) by calling success() 
 * or 2) by calling fail;  
 *
 *
 * @author Deise Kelley
...
public abstract class StateFullBaseParser {
	...
	private class SContainer {
	    public Symbol sym;
	    public int marked; 
	    SContainer(Symbol s){ this.sym = s; marked = 0;}
	    public boolean isMarked(){ return marked > 0;}
	    public void mark(){ marked++;}
	    public void unmark(){ marked = marked > 0 ? marked - 1 : 0;}
	}
